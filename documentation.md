...
**Scope:**
- Application supports educational clinical risk assessment for medical professionals
- Focus limited to heart disease prediction using structured patient data

**Assumptions:**
- Input values are provided correctly by trained medical staff
- Users have access to clinical measurements (blood tests, ECG, etc.)
- Output supports decision-making and does not replace medical diagnosis
...
| 2 | Real-time Clinical User Input | Structured input | Per request | 13 clinical parameters entered by medical professional at inference time |
| 3 | OpenAI GPT-4o-mini API | LLM / Knowledge Base | Per request | External knowledge source for natural language explanation generation |
...
**EDA Key Findings:**
- Dataset is balanced (499 vs 526 samples per class)
- No missing values across all 1025 entries
- Chest pain type (cp) and several cardiovascular indicators (ca, thalach, oldpeak) showed strong relationships with the target variable
- No major outliers detected in key features
...
![Confusion Matrix LR](screenshots/confusion_matrix_lr.png)

**Confusion Matrix – Random Forest:**
![Confusion Matrix RF](screenshots/confusion_matrix_rf.png)
...
See predict() in [`app.py`, lines 213-235](https://github.com/sopasua1/health-risk-assistant/blob/main/app.py#L213-L235)
...
See generate_explanation() in [`app.py`, lines 173-211](https://github.com/sopasua1/health-risk-assistant/blob/main/app.py#L173-L211)
...
Iteration 1 (free text): Generated a continuous paragraph explaining the prediction and patient values.
Iteration 2 (bullet points): Generated structured bullet points highlighting risk probability, key factors and recommendations.
Chosen format: Bullet points were selected because they are easier to read and more suitable for clinical workflows.
